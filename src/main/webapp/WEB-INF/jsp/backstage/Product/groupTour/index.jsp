...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <c:forEach items="${groupTours}" var="groupTour">
        <tr>
            <td>${groupTour.title}</td>
            <td>${groupTour.label}</td>
            <td>${groupTour.location}</td>
            <td><c:choose><c:when test="${groupTour.available == false}">已下架</c:when><c:otherwise>已上架</c:otherwise></c:choose></td>
            <td><fmt:formatDate value="${groupTour.gmtModified}" pattern="yyyy-MM-dd HH:mm:ss"/> </td>
            <td>
                    <a href="/backstage/Product/groupTour/${groupTour.id}/detail">子产品</a>
                <shiro:hasPermission name="product:update">
                    <a href="/backstage/Product/groupTour/${groupTour.id}/update">编辑</a>
                </shiro:hasPermission>
                    <%--<a class="deleteBtn" href="#" data-id="${groupTour.id}">删除</a>这个按钮会导致一些设计上的bug--%>
            </td>
        </tr>
    </c:forEach>
</table>
...
<script>
    $(function () {
        $('#addnew').click(function(){
            window.location.href="/backstage/Product/groupTour/add";
        });
    });
    ...
    $(".deleteBtn").click(function() {
        if(confirm("确定要删除吗？"))
        {
            var url = "/backstage/Product/groupTour/"+$(this).data("id")+"/delete";
