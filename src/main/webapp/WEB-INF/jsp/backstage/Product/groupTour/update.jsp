...
<body>
<form action="/backstage/Product/groupTour/update" method="post" class="definewidth m20">
    <input type="hidden" name="id" value="${groupTour.id}"/>
    <table class="table table-bordered table-hover definewidth m10">
        <tr>
            <td width="10%" class="tableleft">标题</td>
            <td><input type="text" name="title" value="${groupTour.title}"/></td>
        </tr>
        <tr>
            <td class="tableleft">标签</td>
            <td><input type="text" name="label" value="<c:forEach items="${groupTour.label}" var="lab">${lab},</c:forEach>"/></td>
        </tr>
        <tr>
            <td class="tableleft">地点</td>
            <td><input type="text" name="location" value="${groupTour.location}"/></td>
        </tr>
        <tr>
            <td class="tableleft">介绍</td>
            <td><textarea type="text" name="introduce">${groupTour.introduce}</textarea></td>
        </tr>
        <tr>
            <td class="tableleft">行程</td>
            <td><textarea type="text" name="travel">${groupTour.travel}</textarea></td>
        </tr>
        <tr>
                                ...
                                <div class="file-actions">
                                    <div class="file-footer-buttons">
                                        <label class="checkbox inline"> <input type="checkbox" name="picture" value="${picture}"
                                                <c:forEach items="${groupTour.picture}" var="pic">
...
<script>
    $(function () {
        $('#backid').click(function(){
            window.location.href="/backstage/Product/groupTour/index";
        });
